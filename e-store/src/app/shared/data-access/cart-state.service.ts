...
export class CartStateService {
  private _storegeService = inject(StorageService);
  ...
  loadProducts$ = this._storegeService
    .loadProducts()
  ...
  state = signalSlice({
    ...
    selectors: (state) => ({
      count: () =>
        state().products.reduce((acum, product) => acum + product.quantity, 0),
      price: () => {
    ...
    effects: (state) => ({
      load: () => {
        if (state().loaded) {
          this._storegeService.saveProducts(state().products);
        }
